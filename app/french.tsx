...
import { frenchSongs } from '@/data/songs';
...
export default function FrenchScreen() {
  ...
  const filteredSongs = frenchSongs.filter((song) => {
  ...
  const handleSelectSong = (song: Song) => {
    router.push({
      pathname: '/song/[id]',
      params: { id: song.id, returnTo: 'french' },
    } as any);
  };
    ...
    <TouchableOpacity
      ...
      <View style={[styles.iconContainer, { backgroundColor: colors.purple[50] }]}>
        <Ionicons 
          name="book" 
          size={20} 
          color={colorScheme === 'dark' ? colors.foreground : colors.purple[600]} 
        />
      </View>
